...
import { Log, env, email } from "decentraland-commons";
...
export default class ConfigurationService {
  constructor(envName) {
    if (!ENVS.includes(envName)) {
      configLog.error(
        `Error initializing: ${envName} is not in ${JSON.stringify(ENVS)}`
      );
    }
    this.envName = envName;

    env.load({ path: path.resolve(process.cwd(), "src/.env") });
  }
  ...
  startServer() {
    ...
    app.listen(port, () => {
      this.serverLog.info(`Tromazo server running on port ${port}`);
    });
  ...
  get ethereumService() {
    if (!this._ethereum) {
      this._ethereum = new EthereumService(
        env.get("ETHEREUM_PROVIDER", "ws://localhost:8546")
      );
    }
  ...
  setupNotificationTemplate() {
    this.emailService.setTemplate("notification", (opts) => {
      ...
      const infoUrl = `https://tromazo.com/info/${alarmId}`;
      const unsubscribeUrl = `https://tromazo.com/unsubscribe/${alarmId}`;
      const formatPrePost = (pre, post, items) =>
        pre + items.join(post + pre) + post;
      const formatEvent = (event) => {
        const abiEventMatches = opts.alarm.abi.filter(
          (abiEntry) =>
            abiEntry.type === "event" && abiEntry.name === event.event
        );
        if (!abiEventMatches.length) {
          return (
            "(Mismatched ABI Event): " +
            event.event +
            JSON.stringify(event.returnValues)
          );
        }
        const abiEvent = abiEventMatches[0];
        return `${abiEvent.name} (${abiEvent.inputs
          .map(
            (input) =>
              `${input.name} [${input.type}]: ${event.returnValues[input.name]}`
          )
          .join(", ")})`;
      };
...
We detected ${
        eventNumber > 1 ? "new events" : "a new event"
      } regarding the contract ${address} at block ${blockNumber}. The transaction that triggered these events has the hash ${transactionHash}. Here's a summary of the event${
        eventNumber > 1 ? "s" : ""
      }:
...
<p>We detected ${
        eventNumber > 1 ? "new events" : "a new event"
      } regarding the contract ${address} at block ${blockUrl}. The transaction that triggered these events has the hash ${transactionUrl}. Here's a summary of the event${
        eventNumber > 1 ? "s" : ""
      }:</p>
...
`;
      ...
      return {
        ...
        to: opts.alarm.email,
        subject: `[Tromazo] New Event in Contract ${opts.alarm.address}`,
        text: text,
        html: html,
      };
    });
  ...
  get dispatchService() {
    if (!this._dispatch) {
      this._dispatch = new DispatchService(
        this.httpService,
        this.emailService,
        "notification",
        this.receiptModel
