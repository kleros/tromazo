...
export default class AlarmRouter {
  ...
  get postAlarm() {
    return (req, res) =>
      buildResponseSchema(this.alarm.storeNewAlarm(req.body), res);
  }

  get getAlarm() {
    return (req, res) => {
      buildResponseSchema(
        this.alarm.getAlarm(req.params.id).then((alarm) => {
  ...
  get deleteAlarm() {
    return (req, res) =>
      buildResponseSchema(this.alarm.destroyAlarm(req.params.id), res);
  }
}
